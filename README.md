...
- **Import ví metamask và connect**: Tự động import ví (nếu cung cấp 12 seeds trong file data.txt) và kết nối ví với trang dự án.
- **Làm task hằng ngày**: check-in và social X (không làm social Discord).

---

## 🔧 Yêu cầu ban đầu

- **Metamask**: Import sẵn 12 seeds vào Metamask (nếu không cung cấp 12 seeds trong file data.txt). Cần min BNB trên mạng BSC để thựuc hiện check-in.
